...
else { const share=fs.readFileSync(shareIndex,'utf8'); for(const marker of ['class="carousel-track"','id="carousel-previous"','id="carousel-next"','showDirection(activeIndex+1)']) if(!share.includes(marker)) failures.push(`Share site missing ${marker}`); if(share.includes('<iframe')) failures.push('Share site still contains an iframe'); if(share.includes('class="eyebrow"')) failures.push('Share site contains a decorative eyebrow label'); const previewImages=(share.match(/<img src="previews\//g)||[]).length; if(previewImages!==5) failures.push(`Expected 5 carousel preview images, found ${previewImages}`); }
